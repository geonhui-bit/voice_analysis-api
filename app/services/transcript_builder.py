...
from app.services.guard_filter import role_for_segment
...
def build_analysis_meta(
    ...
    for speaker, role in speaker_roles.items():
        ...
        line = f"- {role}({speaker}): baseline {bp:.0f}Hz, {voice_desc}"
        if speaker_profiles and speaker in speaker_profiles:
            prof = speaker_profiles[speaker]
            line += (
                f", 발화 {prof['segment_count']}회·{prof['char_count']}자"
            )
            if prof.get("sparse"):
                line += " [발화 부족: 감정 분석은 특이사항 위주]"
        lines.append(line)
